...
import { useProcurementReport } from '../../../hooks/use-reports';
...
const STATUS_BADGES: Record<string, { bg: string; text: string }> = {
  ...
  SUBMITTED: { bg: 'bg-blue-50', text: 'text-blue-700 border-blue-200' },
  APPROVED: { bg: 'bg-indigo-50', text: 'text-indigo-700 border-indigo-200' },
  PARTIALLY_RECEIVED: { bg: 'bg-amber-50', text: 'text-amber-700 border-amber-200' },
  RECEIVED: { bg: 'bg-emerald-50', text: 'text-emerald-700 border-emerald-200' },
...
export default function ProcurementReportPage() {
  ...
  const { data: reportResponse, isLoading } = useProcurementReport(queryParams);
  ...
  const handleExport = async () => {
    try {
      setIsExporting(true);
      await reportsApi.downloadCsv('procurement', {
        warehouseId,
        status: selectedStatus,
        startDate: dateBounds.startDate,
        endDate: dateBounds.endDate,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Authoritative purchase order pipeline, supplier fulfillment, and inbound order value.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium text-slate-500">Total Procurement Value</p>
          <p className="mt-2 text-2xl font-bold text-blue-600">
            ${summary?.totalValue ?? '0.00'}
          </p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
          <p className="text-xs font-medium text-slate-500">Received / Fulfilled</p>
          <p className="mt-2 text-2xl font-bold text-emerald-600">
            {summary?.receivedCount.toLocaleString() ?? '0'}
          </p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
              {isLoading ? (
                ...
                items.map((item) => {
                  ...
                  return (
                    <tr key={item.purchaseOrderId} className="hover:bg-slate-50/60 transition-colors">
